# RFC: Export structural param facts (`container` + `arm`) in the editor metadata

- **Status:** Proposed (2026-07-02). Engine-side counterpart of the `transon-blockly` UAT
  follow-up "collection/struct inputs" (editor UAT findings #1/#2: structured rule parameters —
  `cond.cases`, `chain.funcs`, `object.fields`, `switch.cases` — currently degrade into
  compositions of generic array/object blocks instead of one coherent block per rule). The
  cross-repo decision (engine-first; no editor-side shape catalog) is recorded in
  `transon-blockly/docs/current-state.md` (Last action, 2026-07-02); the consumer contract update
  (`transon-blockly/docs/metadata-contract.md` §2.2) follows this RFC.
- **Type:** Additive metadata-**export** change only — no new rules, no change to template
  semantics or validation behavior. `METADATA_VERSION` `2.0` → `2.1`.
- **Roadmap:** tracked as R-28 in `docs/ROADMAP.md`, following R-23..R-27 (the editor-metadata
  export + `type`-function RFCs).
- **Relationship to R-24:** completes the R-24 export. R-24 shipped `kind`/`options`/`variants`;
  this RFC stops **dropping** the two structural facts the engine already declares and validates
  internally but never emits: a param's *container shape* and, for arm-lists, the *arm schema*.
...
- Prior art: [`editor-metadata-export.md`](editor-metadata-export.md) (R-23..R-25),
  [`type-function.md`](type-function.md) (R-26..R-27).
